...
func SearchNotesByTitle(c *fiber.Ctx) error {
	searchTitle := c.Params("title")

	var notes []Note
	result := initializers.DB.Where("title LIKE ?", "%"+searchTitle+"%").Find(&notes)
	if err := result.Error; err != nil {
		return c.Status(fiber.StatusBadGateway).JSON(fiber.Map{"status": "fail", "message": err.Error()})
	}

	return c.Status(fiber.StatusOK).JSON(fiber.Map{"status": "success", "data": fiber.Map{"notes": notes}})
}

func SearchNotesByContent(c *fiber.Ctx) error {
	searchTitle := c.Params("content")

	var notes []Note
	result := initializers.DB.Where("content LIKE ?", "%"+searchTitle+"%").Find(&notes)
	if err := result.Error; err != nil {
		return c.Status(fiber.StatusBadGateway).JSON(fiber.Map{"status": "fail", "message": err.Error()})
	}
